...
#include<windows.h>              //TIC TAC TOE Game MADE BY PRIYANSHI GUPTA
...
void game()
{
    ...
    if (choice == 1)
    {
    cout << "     Player 1 Please choose your marker(X or O): ";
    char marker_p1;
    cin >> marker_p1;
    current_player = 1;
    current_marker = marker_p1;
    cout<<"\n";
        for (int i = 0; i < 9; i++)
        {
            ...
            if (slot < 1 || slot > 9)
            {
                cout << "You choose a invalid slot! please try again"<<endl;
                i--;
                continue;
            }
            ...
            if (!Mark_X_O(slot))
            {
                cout << "That slot is occupied! please try again"<<endl;
                i--;
                continue;
            }
            gameboard();
